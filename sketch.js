...
var hero,monster,rope,ground,nightmodebutton,daymodebutton;
var bg,bgNight,bgStarter;

function preload() {
  bg = loadImage("background.png");
  bgStarter = loadImage("daynightchoose.jpeg");
  bgNight = loadImage("Nighttimeimage.png");
}
...
function setup() {
  ...
  nightmodebutton = new Nightmode(100,50,100,45);

  daymodebutton = new Daymode(1300,50,100,45);
...
function draw() {
  background(bgStarter);
  Engine.update(engine);

  if(keyCode===78){
    background(bgNight);
    text("Nightime",210,50,40,40);
    textSize(20);
  }

  if(keyCode===68){
    background(bg);
    text("daytime",1150,50,40,40);
    textSize(20);
  }
  ...
  nightmodebutton.display();
  daymodebutton.display();
}
